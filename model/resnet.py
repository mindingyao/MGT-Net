...
def ResNet34(nInputChannels=3, os=32, pretrained=False):
    model = ResNet(nInputChannels, BasicBlock, [3, 4, 6, 3], os,
                    pretrained=pretrained, model_path='./model/resnet34-333f7ec4.pth')
    return model

def ResNet50(nInputChannels=3, os=32, pretrained=False):
    model = ResNet(nInputChannels, Bottleneck, [3, 4, 6, 3], os,
                    pretrained=pretrained, model_path='./model/resnet50-19c8e357.pth')
    return model


if __name__ == '__main__':
    net = ResNet34(3, 16).cuda()
    inp = torch.randn(2, 3, 512, 512).cuda()

    x1, x2, x3, x4, x5 = net(inp)
